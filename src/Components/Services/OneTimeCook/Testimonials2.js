...
const Testimonials2 = () => {
  ...
  return (
    <section className="bg-white py-16 px-6 md:px-12 overflow-hidden">
      ...
      {/* Quote + Video Section */}
      <div className="flex flex-col md:flex-row items-center justify-center max-w-6xl mx-auto gap-10 mb-20">
        {/* Left Quote */}
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        >
          {testimonials.map((t, index) => (
            ...
            >
              <div className="bg-[#193A36] text-white p-8 md:p-10 rounded-3xl shadow-xl w-full md:w-[75%] lg:w-[60%] transition-transform duration-300 hover:scale-[1.02] hover:bg-white hover:text-black border-2 border-transparent hover:border-green-900">
                <div className="flex items-center mb-4">
                  <img
                    src={t.img}
                    alt={t.name}
                    className="w-14 h-14 md:w-16 md:h-16 rounded-full border-2 border-white mr-4"
                  />
                  <div>
                    <p className="font-semibold text-lg md:text-xl">{t.name}</p>
          ...
          {testimonials.map((_, i) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                current === i ? "bg-green-800 scale-110" : "bg-gray-300"
              }`}
            ></button>
          ))}
        </div>
