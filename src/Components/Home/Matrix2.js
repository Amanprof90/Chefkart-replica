    ...
    <div className="w-full bg-gray-100 py-12">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between px-6 space-y-8 md:space-y-0">
        ...
        <div className="text-center relative">
          <FaUtensils className="absolute text-gray-200 text-6xl -top-6 left-1/2 transform -translate-x-1/2 opacity-40" />
          <h2 className="text-4xl font-bold text-black relative z-10">
            {formatNumber(meals, "+")}
          </h2>
          <p className="text-gray-700 text-sm mt-2">Meals cooked with love</p>
        </div>

        {/* Cooks */}
        <div className="text-center relative">
          <FaUserTie className="absolute text-gray-200 text-6xl -top-6 left-1/2 transform -translate-x-1/2 opacity-40" />
          <h2 className="text-4xl font-bold text-black relative z-10">
            {formatNumber(cooks, "+")}
          </h2>
          <p className="text-gray-700 text-sm mt-2">Verified & Trained Cooks</p>
        </div>

        {/* Households */}
        <div className="text-center relative">
          <FaHome className="absolute text-gray-200 text-6xl -top-6 left-1/2 transform -translate-x-1/2 opacity-40" />
          <h2 className="text-4xl font-bold text-black relative z-10">
            {formatNumber(households, "+")}
          </h2>
          <p className="text-gray-700 text-sm mt-2">Households served</p>
        </div>
